...
export const ImageRoutes = () => {
    ...
    const imageMetadata = useImageMetadata(state?.image_name ?? imageDocument.data?.data?.metadata?.arguments?.["extract-image"]?.image)
    ...
    const graphData = useMemo(() => {
        if (imageDocument.data?.data?.metadata) {
            const os = {
                name: imageDocument.data.data.metadata.os_release.id,
                version: imageDocument.data.data.metadata.os_release.version_id,
                python_version: imageDocument.data.data.metadata.python.major + "." + imageDocument.data.data.metadata.python.minor
            }
    ...
    return (
        <div data-testid="image-loaded">
            <NavigationLayout>
                ...
                <ImageSummary imageMetadata={imageMetadata.data?.data} metrics={metrics} imageDocument={imageDocument.data?.data}/>
            </NavigationLayout>
        </div>
    );
};
