					...
					{Object.keys(dependents).map((change, i) => {
						...
						return (
							<React.Fragment key={change + i}>
								{", " + dependents[change].length + (change === "version" ? " had a version change " : (dependents[change].length === 1 ? " was " : " were ") + change) + " ("}
								{dependents[change].map((p, i, a) => {
									...
									return (
										<Link key={i+text} underline="hover" onClick={() => setSelected(p)}>
											{text}
										</Link>
									);
								})}
